...
export const ProductCard = ({
  ...
  return (
    <motion.div
      ...
      key={product.title}
      className="group/product h-96 w-[30rem] relative flex-shrink-0 border rounded-lg overflow-hidden"
    >
      <Link
        href={product.link}
      ...
      <div className="absolute bottom-4 left-4 opacity-0 group-hover/product:opacity-100 text-white">
        ...
        <Link
          href={product.link}
          className="mt-2 inline-block px-4 py-2 bg-blue-600 text-white text-sm font-semibold rounded-lg hover:bg-blue-700 transition duration-300"
        >
          View Details
        </Link>
      </div>
